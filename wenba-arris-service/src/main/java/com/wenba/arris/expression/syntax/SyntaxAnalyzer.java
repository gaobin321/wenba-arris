...
import com.wenba.arris.expression.lexical.LexicalConstants;
import com.wenba.arris.expression.syntax.function.Function;
import com.wenba.arris.expression.syntax.operator.AssignOperator;
import com.wenba.arris.expression.syntax.operator.Operator;
import com.wenba.arris.expression.tokens.*;
import com.wenba.arris.expression.utils.Stack;
...
public class SyntaxAnalyzer {
	...
	/**
	 * 	GB 原来只支持返回一个结果,改造支持返回多个满足条件结果
	 */
	private List<Valuable> finalResult;
	 ...
	 */
	public List<Valuable> analysis(List<TerminalToken> tokens) throws SyntaxException {
	 ...
	 * 解析表达式
	 * @param tokens Token序列
	 * @param variableInitialValues 初始变量值
	 * @return
	 * @throws SyntaxException
	 */
	public List<Valuable> analysis(List<TerminalToken> tokens, Map<String, Valuable> variableInitialValues)
				throws SyntaxException {
	...
	private int analysisSentence(List<TerminalToken> tokens, int index)
		...
		if(!semanticStack.isEmpty()) {
			...
			finalResult = semanticStack.elements();
		}
		return index;
	}
